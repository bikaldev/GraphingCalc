...
#include "etp.hpp"
double  h=0.0;
double dx=20/(double)(N_X-1);//2z/(N_X-1)
double dy=20/(double)(N_Y-1);//2z/(N_Y-1)
...
void lines(int , int, int, double, double, double, double, sf::RenderWindow &window, WindowSize& k);  // Helper ftn to draw correct line in a cell
//void sendData(std::vector<std::vector<double>>, sf::RenderWindow &window);

void imp_display(sf::RenderWindow &window, WindowSize& k) 
{
	//int k=0;
	...
	for (int i = 0; i < N_X-1; i++)
    {
        for (int j = 0; j < N_Y-1; j++)
        {
			...
			lines(c,i,j,data[i][j],data[i+1][j],data[i][j+1],data[i+1][j+1],window,k);  // draw correct line
        }
...
void draw_one(int n, int i, int j, double a, double b, double c, double d, sf::RenderWindow& window, WindowSize& k){
	...
	/*
	Points p1, p2;
	p1=convertor::implicitConvert(p1);
	p2=convertor::implicitConvert(p2);
	...
	p4.x=p2._x;
	p4.y=p2._y;
	sfLine line(p3,p4,4.f);
    window.draw(line);
	*/

}

void draw_opposite(int n, int i, int j, double a, double b, double c, double d, sf::RenderWindow& window, WindowSize& k){
	double x1 = 0, x2 = 0, y1 = 0, y2 = 0;
...
void lines(int n, int i, int j, double a, double b, double c, double d,sf::RenderWindow& window, WindowSize& k) // Draw correct line
{
   // Your code goes here
   // n is cell type (0-15)
   ...
   // a,b,c,d are function values at cell corners (from data array)
	switch(n){
		...
		case 13:
		case 14:	
					draw_one(n, i, j, a, b, c, d, window, k);
					break;
		case 5:
		case 6:
		case 9:
		case 10:	draw_opposite(n, i, j, a, b, c, d , window, k);
					break;
		case 0:
		case 15:	break;
		default:
