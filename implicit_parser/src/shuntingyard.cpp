...
std::queue<Token*> StringToInfix(std::string s){
    ...
    while (!ps.isEmpty()) {
        //this is the part where a new token is generated
        temp = ps.nextToken();
        switch (ps.getType()) {
            case OPERAND:
    ...
    while (!(infix.empty())){
        ...
        switch (p->TypeOf()){
            ...
            case FUNCTION:{
                ...
                //start poping untill a left bracket is reached.
            case RIGHTBRACKET:{
            ...
            case OPERATOR:{
                //i need better understanding of this part.
...
double EvaluatePostfix(std::queue<Token*> postfix, double x, double y){
    std::stack<Token*> s;
    Token* p;
    ...
    while (!(postfix.empty())){
        ...
        switch (p->TypeOf()){
            ...
            //maybe we can check for the variable here.
            //this is exactly the place to add variables
            case VARIABLE:{
