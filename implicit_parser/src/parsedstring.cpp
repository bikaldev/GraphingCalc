...
// Constructor
ParsedString::ParsedString(std::string s){
    _type = OTHER;
    _s = s;
    _pos = 0;
    _d = 0;
...
std::string ParsedString::nextToken(){
    std::string s;
    TokenType currentType = stringType();
    switch (currentType){
        case OPERAND:{
            // parse before the letter that is not a valid operand
            //this searches specifically for an operand
            //this is needed because operand can be of more than one character
...
void ParsedString::convertToNumber(std::string s){
    _d = stod(s);
}

// Convert letter type string to appropriate token types
TokenType ParsedString::convertToOtherTypes(std::string s){
    //this  part of code needs to be modified such that x and y are different variables.
    if (s == "x" || s == "y")
        _type = VARIABLE;
...
TokenType ParsedString::stringType(){
    //this information comes from the list of possible characters in the header file
