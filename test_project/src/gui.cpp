...
Gui::Gui()
{
	...
	window.create(sf::VideoMode(1280.0f, 720.0f), "Graph Layout", sf::Style::Titlebar|sf::Style::Close);
	...
	zoomOut.setPosition(0.95 * VIEW_WIDTH, 0.10 * VIEW_HEIGHT);
...
void Gui::main()
{
	...
	while (window.isOpen())
	{
		...
		{
			graph.draw(window);
			if (error_in_equation == false && error_in_points == false)
			{
				error_msg = "";
			}
			else
...
void Gui::draw_equations()
{
	...
	if (btn1.clicked == true)
	{
		...
		try
		{
			graph.addCurve(tb1->getString(), 1);
			error_msg = "";
			error_in_equation = false;
		}
		catch (FORMATERROR e)
		{
			//std::cout<<"this block is working"<<endl;
			error_msg = e.message();
			error_in_equation = true;
		}
	}
	if (btn2.clicked == true)
	{
