...
{
	t_string = "";
	t_font.loadFromFile("assets/arial.ttf");
...
unsigned int Textbox::getStringLength() const
{
	return t_string.length();
}
void Textbox::setType(Type type)
{
	t_type = type;
}
...
void Textbox::listenForClick(sf::RenderWindow& window)
{
	if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left))
	{
		...
		if (mouse_position.x >= t_position.x && mouse_position.x <= (t_position.x + t_size.x) && mouse_position.y >= t_position.y && mouse_position.y <= (t_position.y + t_size.y))
		{
			t_has_focus = true;
		}
		else
...
void Textbox::listenForText(sf::Event& event)
{
	if (t_has_focus && t_string.length() <= t_limit)
	{
		...
		if (event.type == sf::Event::TextEntered)
		{
			...
			{
				if (t_type == Type::Letter || t_type == Type::AlphaNumeric || t_type == Type::Email || t_type == Type::Any)
				{
			...
			{
				if (t_type == Type::Number || t_type == Type::AlphaNumeric || t_type == Type::Any || t_type == Type::Email || t_type == Type::IP)
				{
					t_string.insert(t_cursor_position, 1, char(code));
					t_cursor_position++;
				}
			...
			{
				if (t_type == Type::IP || t_type == Type::Email || t_type == Type::Any)
				{
					t_string.insert(t_cursor_position, 1, char(code));
					t_cursor_position++;
				}
			...
			{
				if (t_type == Type::Email || t_type == Type::IP || t_type == Type::Any)
				{
					t_string.insert(t_cursor_position, 1, char(code));
					t_cursor_position++;
				}
			...
			{
				if (t_type == Type::Letter || t_type == Type::AlphaNumeric || t_type == Type::Any)
				{
					t_string.insert(t_cursor_position, 1, char(code));
					t_cursor_position++;
				}
